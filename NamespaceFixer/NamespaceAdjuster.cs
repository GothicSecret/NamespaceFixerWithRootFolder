...
namespace NamespaceFixer
{
    internal sealed class NamespaceAdjuster
    {
        ...
        private void MenuItemCallback2(object sender, EventArgs e)
        {
            ...
            allPaths.ToList().ForEach(f => FixNamespace2(f, basePath));
        }
        ...
        private void FixNamespace2(string filePath, string basePath)
        {
            if (!File.Exists(filePath))
            {
                return;
            ...
            var fileContent = File.ReadAllText(filePath);
            var desiredNamespace = _namespaceBuilder.GetIdealNamespace2(filePath, basePath);
